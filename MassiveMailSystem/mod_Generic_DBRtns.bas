...
Public Function DB_CheckTable(ByVal TableName As String)
    ...
    Set RS = DBConn.Execute("SELECT TOP 1 * FROM " & TableName)
    Set RS = Nothing
...
ErrHandler:
    ...
    Else
        MsgBox "Errore durante la connessione al DSN." & vbNewLine & vbNewLine & (Err.Description), vbCritical, "DB Connection:"
    End If
...
ErrHandler:
    MsgBox Purge_ErrDescr(Err.Description), vbExclamation, "Execute Query:"
...
ErrHandler:
    ...
    If Err.Description <> "" Then
        ...
        MsgBox Purge_ErrDescr(Err.Description), vbExclamation, "GetValueById:"
    End If
